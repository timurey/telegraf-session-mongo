      ...
      get(target, property, receiver) {
        switch(property)
        {
          ...
          case '__changeset':
          {
            ...
            //Checking the setting of unsetted keys
            
            Object.keys($unset).forEach((itemSet, iSet, arrSet) => {
              Object.keys($unset).forEach((itemUnset, iUnset, arrUnset) => {
                if(itemSet == itemUnset)
                delete $unset[itemUnset]
              })
            })

            if(Object.keys($set).length > 0) { Object.assign(ret, { $set }); }
            if(Object.keys($unset).length > 0) { Object.assign(ret, { $unset }); }
            Object.keys($set).forEach((item, i, arr) => {
              debug(`Set: ${item}`);
            })
